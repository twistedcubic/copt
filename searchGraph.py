"""
Filter out candidates using vectors created from COPT and spectral methods. Then classify with GW on the reduced number of candidates.
"""
import _init_paths
import utils
import graph
...
def classify_svm_var_len(dataset, data_idx, query_idx, args, node_labels, q_labels, dataset_cls=None, tgt_cls=None, data_sketch=None, C_opt=None):
    ...
    for i, sketch in enumerate(data_sketch):
        ...
        '''
        prob_labels = np.zeros(s , )
        ones = torch.ones()
        prob_labels.scatter_add(index= , src=ones, dim=1)
        '''
    ...
    if C_opt is None and do_grid_search:
        ...
        if use_lap:
            ...
            #gk = grakel.GraphKernel(kernel=[{"name": "weisfeiler_lehmann"}], normalize=True) 
            G2 = []
            for g in all_graphs:
                node2idx = {}
...
def sketch_dataset(dataset, node_labels, args):
    """
    ...
    normalize_Ly = False # True #can try Normalize if e.g. dataset is very heterogeneous
...
def classify_mutag_l1(dataset, queries, args, node_labels, q_labels, dataset_cls=None, tgt_cls=None):#dataset_cls, target, args):
    """
    For classifying real world datasets, e.g. enzymes or mutag dataset.
...
def classify_filter(dataset, queries, args, dataset_cls=None, tgt_cls=None):#dataset_cls, target, args):
    """
    Initial dataset filtering to filter out candidates for a slower but likely more accurate method, e.g. GW distance. 
    """
    #args.alpha = 0.8
    #with open(args.graph_fname, 'rb') as f:
    #    graphs = pickle.read(f)
...
def test_sketch_real_data(args):
    args.n_epochs = 350
    args.k = 1
    ...
    if os.path.exists(sketch_path):
        ...
        print('Creating and caching data!')
...
def test_sketch_syn_data(args):    
    ...
    args.n_per_cls = 10 #130 #100 #20 #15 #5 #15 # 20 #5
    ...
    if not args.lr_hike:
        print('NOTE: for best (and faster) results, consider settings args.hike to True. Achieved by running with "--hike" argument')
    ...
    #sketch query and dataset together
    args.sketch_together = True #False
    ...
    if args.sketch:
        ...
        n_val = len(dataset1) // 4 #200 #20
        n_data = len(dataset1) - n_val #600 #200
    ...
    if args.sketch or args.sketch_together:
    ...
    test_retrieval = True
    if test_retrieval:
        args.k = 3 #3 #40 #24 #100 #@50 #50 #100
        #this is used for testing l2-based retrieval
        print('Testing retrieval. First reduce graph down using with spectral method or COPT sketching, then do fast nearest-neighbor search. args.k: {}'.format(args.k))
        ...
        if test_svd:
            ...
            #need to set args.sketch_together to True to filter
            assert args.sketch_together
            print('Using COPT to reduce graph.')
            cur_idx = classify_filter(dataset_lo, val_set_lo, args, dataset_cls=dataset_cls, tgt_cls=val_cls)  #dataset_cls=None, tgt_cls=None):#dataset_cls, target, args):
...
if __name__ == '__main__':
    ...
    test_sketching_real = False 
    #test_sketching_real = True
    print('NOTE: can toggle between testing on synthetic data or real data.')
    if test_sketching_real:
        print('Testing on real data!')
        test_sketch_real_data(args)
    else:
        print('Testing on synthetic data!')
        test_sketch_syn_data(args)
